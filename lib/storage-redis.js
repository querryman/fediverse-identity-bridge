...
/* -------------------------------------------------------
 * Helper: Batch fetch credentials using MGET (reduces Redis ops)
 * ------------------------------------------------------- */
async function _batchGetCredentials(ids) {
  if (!ids || ids.length === 0) return [];
  
  try {
    const c = await ensureClient();
    const keys = ids.map(id => `${NS}:vc:${id}`);
    const jsons = await c.mGet(keys);  // ← Single MGET instead of N individual GETs
    
    return jsons.map((json, idx) => {
      try {
        return json ? JSON.parse(json) : null;
      } catch (e) {
        console.error(`[storage-redis] Parse error for VC ${ids[idx]}: ${e.message}`);
        return null;
      }
    }).filter(Boolean);
  } catch (err) {
    console.error(`[storage-redis] _batchGetCredentials failed: ${err.message}`);
    throw err;
  }
}
...
async function getVCsByIssuer(issuerDid) {
  ...
  try {
    ...
    return await _batchGetCredentials(ids);  // ← Use MGET instead of N individual GETs
  } catch (err) {
